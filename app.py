...
years = st.sidebar.slider("比較年数（年）", 10, 50, 35, key="base_years")

st.sidebar.subheader("📈 インフレ設定")
inflation_rate = st.sidebar.slider("インフレ率（％）", 0.0, 5.0, 1.0, key="inflation")
...
apt_utility = st.sidebar.number_input("光熱費（万円/月）", value=1.0, key="apt_utility")

st.sidebar.markdown("**💰 マンションの住宅ローン控除**")
apt_kojo_years = st.sidebar.number_input("控除年数（年）", value=10, key="apt_kojo_years")
apt_kojo_rate = st.sidebar.number_input("還元率（％）", value=1.0, key="apt_kojo_rate")

# -------------------
# 一条設定
# -------------------
st.sidebar.subheader("🏠 一条工務店の設定")
ichijo_loan = st.sidebar.number_input("借入額（万円）", value=5000, key="ichijo_loan")
ichijo_loan_years = st.sidebar.number_input("返済期間（年）", value=35, key="ichijo_loan_years")
...
ichijo_solar_income = st.sidebar.number_input("太陽光売電収入（万円/月）", value=0.5, key="ichijo_solar_income")
ichijo_repair_year = st.sidebar.number_input("定期修繕実施年", value=20, key="ichijo_repair_year")
ichijo_repair_cost = st.sidebar.number_input("定期修繕費用（万円）", value=100, key="ichijo_repair_cost")

st.sidebar.markdown("**💰 一条の住宅ローン控除**")
ichijo_kojo_years = st.sidebar.number_input("控除年数（年）", value=10, key="ichijo_kojo_years")
